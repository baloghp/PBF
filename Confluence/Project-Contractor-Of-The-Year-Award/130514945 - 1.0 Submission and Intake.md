...
- Page ID: `130514945`
- URL: `https://ittd.atlassian.net/wiki/spaces/PBF/pages/130514945/1.0+Submission+and+Intake`
- Confluence version: `2026-06-29T21:18:21.754Z`
...
1. **Nominee initiates nomination**
    ...
    * Starts a nomination. **The same account may start further nominations** (DR-003). Each nomination is a distinct project with its own packet and client evaluation. The same contracted engagement cannot be entered twice.
...
flowchart TD
    ...
    J -->|Yes| L[Assign category by headcount]
    L --> N[Mark Ready for Assessment]
    N --> M[PM Assigns Stage 1 Assessors]
...
5. **Assign category (intake, not Stage 2)** — PM / Award Operations sets Small / Medium / Large from customer-project headcount (DR-003)
6. **Mark Ready for Assessment** — PM assigns assessors (≥2 per nomination, COI-checked)
7. **Application lifecycle visibility** — dashboard status updated throughout
8. **Respond to applicant questions** — program support / PMO inbox (not a volunteer coach)
